...
public class Node {
    ...
    private String id;
    private boolean state[];        //since the state is an array and the size wont be changing we save its length to a variable
    ...
    public void newChild(){
        ...
        //if the torch is rightside of the bridge,only people from the right side can cross
        //and we also know that people who cross the left to right there is going to be pair of people crossing 
        //and only one person if the torch is located leftside
        
        //also nodes wont be visited twice since this only happens when 2 people from the left want to cross the bridge together 
        //but this is not efficient for our solution
        //so knowing that the childrens or the subTrees will be creates like so 

        //if the torch is located in the left side of the bridge we create all the possible pairs
            ...
            for(int i = 0; i < stateLen - 1 ; i++){
                for(int j = i + 1; j < stateLen -1 ; j++){
                    ...
                    if(state[i] == false && state[j] == false){
                        ...
                        //and the id of the new node will simple be the combination of people that crossed
        ...
        if(state[stateLen - 1] == true){
            for(int i = 0; i < stateLen -1 ; i++){ 
                //and we sent across only people who are alredy located to the right side of the bridge
                if(state[i] == true){
                    boolean stateOfTheChild[] = new boolean[stateLen];
